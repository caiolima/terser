...
export async function run_cli({ program, packageJson, fs, path }) {
    ...
    async function run_cli() {
        ...
        const { ScopeInfoBuilder, encode: encodeScopes } = await import("@chrome-devtools/source-map-scopes-codec");
        const scopeInfoBuilder = new ScopeInfoBuilder();
        let result;
        try {
            result = await minify(files, options, fs, scopeInfoBuilder);
        } catch (ex) {
        ...
        if (program.output == "ast") {
            ...
            let map = result.map;
            if (options.sourceMap && options.sourceMap.url !== "inline" && map) {
                if (options.sourceMap.scopes) {
                    map = JSON.stringify(encodeScopes(scopeInfoBuilder.build(), JSON.parse(map)));
                }
                fs.writeFileSync(program.output + ".map", map);
            }
        } else {
