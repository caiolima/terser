...
async function minify(files, options) {
    ...
    // After all transformations, re-establish scope_to_index mappings.
    // Compression and figure_out_scope() create new block_scope objects,
    // invalidating the references stored during capture().
    if (scope_map && (options.compress || options.mangle)) {
        if (!options.mangle) {
            // figure_out_scope wasn't called after compression; call it now
            // so block_scope objects and variables maps are up to date.
            toplevel.figure_out_scope({});
        }
        scope_map.remap(toplevel);
    }
