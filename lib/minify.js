...
import {
    process_original_scopes,
    open_file_scope_ranges,
    close_file_scope_ranges,
} from "./sourcemap-scopes.js";
import * as sourceMapScopesCodec from "@chrome-devtools/source-map-scopes-codec";
...
function* minify_sync_or_async(files, options, _fs_module, _codec_module) {
    const { ScopeInfoBuilder, encode: encodeScopes } = _codec_module;
    const scopeInfoBuilder = new ScopeInfoBuilder();
        ...
        if (options.parse.spidermonkey) {
            ...
            for (var name in files) if (HOP(files, name)) {
                ...
                if (options.sourceMap && options.sourceMap.scopes) {
                  ...
                  process_original_scopes(options.parse, scopeInfoBuilder);
                }
            }
        }
    ...
    if (!HOP(options.format, "code") || options.format.code) {
        ...
        if (options.sourceMap && options.sourceMap.scopes) {
            format_options.scope_info_builder = scopeInfoBuilder;
        }
        var stream = OutputStream(format_options);
        if (options.sourceMap && options.sourceMap.scopes) {
            open_file_scope_ranges(stream, scopeInfoBuilder);
        }
        toplevel.print(stream);
        if (options.sourceMap && options.sourceMap.scopes) {
            close_file_scope_ranges(stream, scopeInfoBuilder);
        }
        ...
        if (options.sourceMap) {
            Object.defineProperty(result, "map", {
                ...
                enumerable: true,
                get() {
                    let map = format_options.source_map.getEncoded();
                    if (options.sourceMap.scopes) {
                        map = encodeScopes(scopeInfoBuilder.build(), map);
                    }
...
async function minify(files, options, _fs_module) {
    const gen = minify_sync_or_async(files, options, _fs_module, sourceMapScopesCodec);
