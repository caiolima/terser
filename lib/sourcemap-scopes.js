...
export function process_generated_ranges(toplevel, scopeInfoBuilder) {
  ...
  for (const scope of scopeInfoBuilder.fileScopes) {
    ...
    scopeInfoBuilder.startRange(toplevel.gen_start.line - 1, toplevel.gen_start.col, { scope, values });
  }
...
function startRangeForNode(node, values, scopeInfoBuilder) {
  ...
  if (node.original?.callsite) {
    ...
    // console.error("call site added: " + JSON.stringify(callSite));
  }
  // console.error("start range");
  scopeInfoBuilder.startRange(node.gen_start.line - 1, node.gen_start.col, { scope: node.original?.scope, values, callSite });
}
