...
function ScopeMap(options) {
    ...
    function encode(source_map_json) {
        ...
        // Build GeneratedRange tree - sort by start position and nest properly
        ...
        sorted_ranges.forEach((range) => {
            ...
            // Pop ranges from stack that this range is not inside
            while (range_stack.length > 0) {
                var parent = range_stack[range_stack.length - 1];
                var inside = (range.start.line > parent.end.line ||
                    (range.start.line === parent.end.line && range.start.column >= parent.end.column));
                if (inside) {
                    range_stack.pop();
                } else {
                    break;
            ...
            if (range_stack.length > 0) {
                range_stack[range_stack.length - 1].children.push(encoded_range);
            } else {
                top_level_ranges.push(encoded_range);
            }
            range_stack.push(encoded_range);
        });
