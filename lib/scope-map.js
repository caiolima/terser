...
function ScopeMap(options) {
    ...
    function capture(toplevel) {
        ...
        var tw = new TreeWalker((node, descend) => {
            ...
            if (node.is_block_scope() && node.block_scope
                ...
                // Use parent_scope to find the correct parent in the scope hierarchy.
                // AST_Catch's block_scope.parent_scope skips the AST_Try scope,
                // making try and catch siblings rather than parent-child.
                var parent_scope_node = node.block_scope.parent_scope;
                var parent_index = parent_scope_node
                    ? scope_to_index.get(parent_scope_node)
                    : undefined;
                if (parent_index !== undefined) {
                    original_scopes[parent_index].children.push(scope_info);
    ...
    function exit_scope(node, line, col) {
        // Guard: only pop if this node was actually entered.
        // enter_scope returns early when the node isn't in scope_to_index,
        // so we must not pop a parent's range by mistake.
        var index = scope_to_index.get(node);
        if (index === undefined) return;
    ...
    // Re-establish scope_to_index after compression/mangling.
    // figure_out_scope() creates new block_scope objects on each call,
    // invalidating the references stored during capture(). This function
    // walks the post-transformation AST and matches scopes by original
    // source position to restore the mapping.
    function remap(toplevel) {
        scope_to_index.clear();

        var tw = new TreeWalker((node, descend) => {
            // Block scopes
            if (node.is_block_scope() && node.block_scope
                && node.block_scope.variables && node.block_scope.variables.size > 0) {
                var start_line = node.start.line - 1;
                var start_col = node.start.col;
                for (var i = 0; i < original_scopes.length; i++) {
                    var scope_info = original_scopes[i];
                    if (scope_info.kind === "block"
                        && scope_info.start.line === start_line
                        && scope_info.start.column === start_col) {
                        scope_to_index.set(node.block_scope, scope_info.index);
                        break;
                    }
                }
            ...
            // Function/global scopes
            if (node instanceof AST_Scope && !node.is_block_scope()) {
                var kind = node instanceof AST_Toplevel ? "global" : "function";
                var start_line = node.start.line - 1;
                var start_col = node.start.col;
                for (var i = 0; i < original_scopes.length; i++) {
                    var scope_info = original_scopes[i];
                    if (scope_info.kind === kind
                        && scope_info.start.line === start_line
                        && scope_info.start.column === start_col) {
                        scope_to_index.set(node, scope_info.index);
                        break;
